...
{
    QMessageBox::about(this, "关于本软件", "欢迎使用我的智能语音记事本，该记事本能够进行下列操作：新建文档、编辑文档、文件保存、文件另存为、语音输入、语音朗读。");
}
...
{
    ui->statusBar->showMessage("语音输入功能被触发", 2000);
}

// 实现语音朗读槽函数
void MainWindow::on_speedButton_clicked()
{
    ui->statusBar->showMessage("语音朗读功能被触发", 2000);
}

void MainWindow::initWindow()
{
    setWindowTitle("智能语音记事本");
    ui->mainToolBar->setWindowTitle("工具栏");
    ui->mdiArea->setHorizontalScrollBarPolicy(Qt::ScrollBarAsNeeded);
    ui->mdiArea->setVerticalScrollBarPolicy(Qt::ScrollBarAsNeeded);
    ui->statusBar->showMessage("欢迎使用智能语音记事本");

    QLabel *label = new QLabel(this);
    label->setFrameStyle(QFrame::Box | QFrame::Sunken);
    ui->statusBar->addPermanentWidget(label);

    ui->actionNew->setStatusTip("创建一个文件");
    ui->actionOpen->setStatusTip("打开一个已经存在的文件");
    ui->actionSave->setStatusTip("保存文档到硬盘");
    ui->actionSaveAs->setStatusTip("以新的名称保存文档");
    ui->actionExit->setStatusTip("退出应用程序");
    ui->actionUndo->setStatusTip("撤销先前的操作");
    ui->actionRedo->setStatusTip("恢复先前的操作");
    ui->actionCut->setStatusTip("剪切选中的内容到剪贴板");
    ui->actionCopy->setStatusTip("复制选中的内容到剪贴板");
    ui->actionPaste->setStatusTip("粘贴剪贴板的内容到当前位置");
    ui->actionClose->setStatusTip("关闭活动窗口");
    ui->actionCloseAll->setStatusTip("关闭所有窗口");
    ui->actionTile->setStatusTip("平铺所有窗口");
    ui->actionCascade->setStatusTip("层叠所有窗口");
    ui->actionNext->setStatusTip("将焦点移动到下一个窗口");
    ui->actionPrevious->setStatusTip("将焦点移动到前一个窗口");
    ui->actionAbout->setStatusTip("显示本软件的介绍");
}
